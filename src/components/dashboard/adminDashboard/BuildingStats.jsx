...
const BuildingStats = ({ buildingsStats }) => {
  return (
    <>
      <div className="max-w-[440px] mt-6 mb-7">
        {buildingsStats && buildingsStats.length > 0 ? (
          buildingsStats.map((stats, index) => {
            const { name, buildings } = stats;
            return (
              <>
                <div
                  key={index}
                  className="w-full p-4 bg-white border border-[#0000001A] drop-shadow-sm rounded-lg"
                >
                  <div className="flex justify-between items-center">
                    <span className="font-bold mt-2 ms-1 text-[19px]">
                      {name}
                    </span>
                  ...
                  <div className="w-full ">
                    {buildings &&
                      buildings.map((building, index) => {
                        const { name, annualRentAsPerContract, actualRent } =
                          building;
                        return (
                          <>
                            <div
                              key={index}
                              className="flex justify-between ps-1 items-center"
                            >
                              <span className="font-semibold mt-2 text-[16px]">
                                {name}
                              </span>
                            </div>
                            <div className="mt-1 mb-4 w-full rounded-lg border border-dashed border-[#0000001A] p-2">
                              <div className="grid mt-4 lg:grid-cols-2 gap-4">
                                ...
                                <div className="flex flex-col w-full">
                                  ...
                                  <span className="text-[16px] font-semibold">
                                    {formatAmount(annualRentAsPerContract)}
                                  </span>
                                </div>
                              </div>
                            </div>
