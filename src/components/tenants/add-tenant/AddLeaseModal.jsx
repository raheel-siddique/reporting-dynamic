...
import { LeaseStatuses, LeaseStatusesReverse } from "../../../utils/enums";
import { useEffect } from "react";
import CustomImageUploader from "../../img/CustomImageUploader";
...
const AddLeaseModal = ({
  ...
  useEffect(() => {
    if (initialValues?.contractFile)
      setDocumentPaths(
        initialValues.contractFile.split(",").map((path) => path.trim())
      );
          ...
          <div>
                ...
                leases: [
                  {
                    ...
                    leaseStatusText: LeaseStatusesReverse[initialValues?.leaseStatus],
                  } || {
                    startDate: null,
                    endDate: null,
            ...
            >
              {({ values, setFieldValue }) => {
                useEffect(() => {
                  values.leases.forEach((lease, index) => {
                    const { startDate, endDate, gracePeriodEndDate } = lease;

                    if (!startDate || (!endDate && !gracePeriodEndDate)) {
                      // If start date is removed or both end dates are removed, set noOfdays to empty
                      setFieldValue(`leases.${index}.noOfdays`, "");
                    } else {
                      const start = new Date(startDate);
                      const end = new Date(gracePeriodEndDate || endDate);
                      const diffDays = Math.ceil(
                        (end - start) / (1000 * 60 * 60 * 24)
                      );

                      setFieldValue(`leases.${index}.noOfdays`, diffDays);
                    }
                  });
                }, [values.leases, setFieldValue]);
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                  ...
                                  <div>
                                    ...
                                    <div className="custom-select">
                                      <ReusableInput
                                        type={"text"}
                                        inpName={`leases.${index}.leaseStatusText`}
                                        inpPlaceholder="Lease Status"
                                        readOnly={viewOnly}
                                        showDisabledClasses={false}
                                      />
                                      {/* <Field
                                        ...
                                        //   defaultValue={leaseStatus[0].name} // Setting default value to "Pending"
                                      >
                                        <option value="">
                                          Select lease Status
                                        </option>
                                  ...
                                  <div>
                                    ...
                                    <ReusableInput
                                      ...
                                      inpPlaceholder="Enter Actual Rent"
                                      readOnly={viewOnly}
                                      showDisabledClasses={false}
                                    />
                                    <ErrorMessage
