...
export function generateReceiptColumns({ includeStatus = false }) {
  const columns = [
    { Header: "#", accessor: "id" },
    ...
    },
    {
      ...
      accessor: "actions",
      Cell: ({ row }) => (
        <img className="cursor-pointer" src={PdfIcon} alt="download pdf icon" />
      ),
    },
  ];
    ...
    columns.splice(7, 0, {
      ...
      Cell: ({ row }) => (
        <span
          className={`
            ${
              row.original.status.toLowerCase() === "deposited"
